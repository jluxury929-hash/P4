/**
 * ===============================================================================
 * APEX MASTER v79.1 (TRIANGLE FINALITY) - THE 100% CERTAINTY BUILD
 * ===============================================================================
...
if (cluster.isPrimary) {
    ...
    console.log(`║   ⚡ APEX TITAN v79.1 | TRIANGLE FINALITY ENGAGED    ║`);
    ...
    async function ignite() {
        for (const url of GLOBAL_CONFIG.RPC_POOL) {
            try {
                ...
                const cbeth = new Contract(GLOBAL_CONFIG.CBETH, erc20, wallet);

                // Ensure all vector tokens are pre-approved for the sniper contract
                const tokens = [weth, usdc, cbeth];
                for (const token of tokens) {
                    const allowance = await token.allowance(wallet.address, GLOBAL_CONFIG.TARGET_CONTRACT).catch(() => 0n);
                    if (allowance < parseEther("1000")) {
                        await (await token.approve(GLOBAL_CONFIG.TARGET_CONTRACT, ethers.MaxUint256)).wait();
                    }
                }
...
async function executeAbsoluteStrike(provider, wallet, iface) {
    try {
        ...
        // Forced Dynamic Data (Triangle Vector A: WETH -> USDC -> cbETH -> WETH)
        const path = [GLOBAL_CONFIG.WETH, GLOBAL_CONFIG.USDC, GLOBAL_CONFIG.CBETH, GLOBAL_CONFIG.WETH];
        const data = iface.encodeFunctionData("executeTriangle", [path, parseEther("25")]);

        const sim = await provider.call({ to: GLOBAL_CONFIG.TARGET_CONTRACT, data, from: wallet.address, gasLimit: 1300000n }).catch(() => "0x");
        if (sim === "0x") return; 
        ...
        console.log(`${TXT.green}🚀 TRIANGLE STRIKE FIRED! (Nonce: ${nonce})${TXT.reset}`);
    } catch (e) { }
}
