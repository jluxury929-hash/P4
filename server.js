// ===============================================================================
// APEX OMNISCIENT TRIANGLE v37.0 (ULTIMATE MERGE) - HIGH-FREQUENCY CLUSTER
// ===============================================================================
...
const GLOBAL_CONFIG = {
    ...
    GAS_LIMIT: 1300000n,                 // v23.0 Optimized Buffer
...
if (cluster.isPrimary) {
    ...
    console.log(`${TXT.bold}${TXT.gold}║   ⚡ APEX OMNISCIENT MASTER v37.0 | CLUSTER ENGINE    ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}║   DUAL: WHALE SNIPER + TRIANGULAR ARBITRAGE            ║${TXT.reset}`);
    console.log(`${TXT.bold}${TXT.gold}╚════════════════════════════════════════════════════════╝${TXT.reset}\n`);
...
async function initWorker(CHAIN) {
    ...
    try {
        const server = http.createServer((req, res) => {
            if (req.url === '/status') {
                ...
                res.end(JSON.stringify({ status: "ACTIVE", mode: "OMNISCIENT_TRIANGLE", chain: CHAIN.name }));
    ...
    // A. PENDING SNIPER (Whale Intercept + Triangle Probe)
    wsProvider.on("pending", async (txHash) => {
        try {
            ...
            // Trigger 2: Triangle Volatility Probe (Stochastic)
    ...
    try {
        ...
        if (pool && CHAIN.chainId === 8453) {
            try {
                ...
                const poolCap = BigInt(res0) / 10n; // 10% limit
