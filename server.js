// ===============================================================================
// APEX OMNISCIENT TRIANGLE v60.0 (ULTIMATE STABILITY) - HIGH-FREQUENCY ENGINE
// ===============================================================================
...
process.on('uncaughtException', (err) => {
    ...
    // v60.0: Aggressive suppression of RPC Handshake noise to prevent Railway reboots
...
const GLOBAL_CONFIG = {
    ...
    // 🚦 TRAFFIC CONTROL (v60.0 INFINITE STABILITY)
    MEMPOOL_SAMPLE_RATE: 0.01,           // 1% per core (Full coverage across 48-core system)
    WORKER_BOOT_DELAY_MS: 60000,         // 60s Master Queue (Necessary for Infura handshake safety)
    HEARTBEAT_INTERVAL_MS: 300000,       // 5m Heartbeat (Near-zero background load)
    RPC_COOLDOWN_MS: 60000,              
    RATE_LIMIT_SLEEP_MS: 900000,         // 15m Ghost Mode if 429 persists
    
    // 🐋 OMNISCIENT & TRIANGLE SETTINGS (v37.0)
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v60.0 | INFINITE STABILITY OVERLORD  ║
║   STRATEGY: ZERO-PROBE HANDSHAKE + TRI-DIVISION      ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);

    const cpuCount = Math.min(os.cpus().length, 48); 
    console.log(`${TXT.cyan}[SYSTEM] Initializing 48-Core Fleet (60s Queue Stagger)...${TXT.reset}`);
    ...
    cluster.on('exit', (worker) => {
        console.log(`${TXT.red}⚠️ Core Desynced. Cooldown Resting (600s)...${TXT.reset}`);
        setTimeout(() => cluster.fork(), 600000);
    });
} 
// --- WORKER PROCESS ---
else {
    ...
    // v60.0 Linear Stagger: Ensures individual core handshakes never overlap
    const startDelay = (cluster.worker.id % 24) * 15000;
    setTimeout(() => {
...
async function initWorker(CHAIN) {
    ...
    // Division Assignment: 0 = Sniper, 1 = Triangle, 2 = Monitor
    ...
    async function safeConnect() {
        if (retryCount >= 200) return;

        try {
            // v60.0 FIX: Pre-form hardcoded Network object to BYPASS eth_chainId Probe
            ...
            const ROLE = ["SNIPER", "TRIANGLE", "MONITOR"][DIVISION];
            ...
            // v60.0 SHARDED SUBSCRIPTION LOGIC
            setTimeout(() => {
                if (DIVISION === 0) {
                    // DIVISION A: MEMPOOL WHALE SNIPING
                    wsProvider.on("pending", async (txHash) => {
                        ...
                        try {
                            ...
                            if (tx && tx.to && tx.value >= GLOBAL_CONFIG.WHALE_THRESHOLD) {
                                ...
                                if (isDEXTrade) {
                                    console.log(`\n${TAG} ${TXT.gold}⚡ WHALE INTERCEPT: ${formatEther(tx.value)} ETH whale!${TXT.reset}`);
                    ...
                    // DIVISION B: TRIANGLE PROBING (STOCHASTIC)
                    setInterval(async () => {
                        if (isProcessing) return;
                        try {
                            isProcessing = true;
                ...
                // DIVISION 2: MONITOR (PASSIVE SYNC)
            }, (cluster.worker.id % 48) * 10000); // 10s subscription stagger per worker
